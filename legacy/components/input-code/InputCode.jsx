...
function generateInputId() {
  return 'rshb-legacy-input-code-' + (++idCounter);
}
...
function getDigitsOnly(value) {
  return String(value).replace(/\D/g, '').slice(0, CODE_MAX_DIGITS);
}
...
export default function InputCode(props) {
  ...
  const resolvedLabel = label != null ? label : 'Проверочный код';
  ...
  const [uncontrolledValue, setUncontrolledValue] = useState(function () {
    return getDigitsOnly(defaultValue != null ? defaultValue : '');
  });
  ...
  const resolvedValue = isControlled ? getDigitsOnly(value != null ? value : '') : uncontrolledValue;
  ...
  return (
    <div
      className={joinClassNames('rshb-legacy-input-text', 'rshb-legacy-input-code', className)}
      ...
      <div className="rshb-legacy-input-text__field">
        ...
        <div className="rshb-legacy-input-text__content">
          ...
          <input
            ...
            className="rshb-legacy-input-text__control rshb-legacy-input-code__control"
            disabled={resolvedDisabled}
            id={inputId}
            inputMode="numeric"
          ...
          />
          {shouldShowMask ? (
            <span aria-hidden="true" className="rshb-legacy-input-code__mask">
              <span className="rshb-legacy-input-code__mask-filled">{filledMaskValue}</span>
              <span className="rshb-legacy-input-code__mask-placeholder">{remainingMaskValue}</span>
            </span>
          ) : null}
        </div>
        {showsClearControl ? (
