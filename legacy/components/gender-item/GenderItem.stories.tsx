...
import type { LegacyGenderItemProps } from './index.js';
import GenderItem from './index.js';

const meta = {
  title: 'Legacy/Components/GenderItem',
...
const columns: Array<{ label: string; props: Pick<LegacyGenderItemProps, 'label' | 'position'> }> = [
...
function stateProps(variant: MatrixVariant): Partial<LegacyGenderItemProps> {
  if (variant === 'active') return { checked: true, previewState: 'default', readOnly: true };
  if (variant === 'error') return { invalid: true, previewState: 'default', readOnly: true };
  if (variant === 'error-hover') return { invalid: true, previewState: 'hover', readOnly: true };
  if (variant === 'focused') return { previewState: 'focused', readOnly: true };
  if (variant === 'hover') return { previewState: 'hover', readOnly: true };
