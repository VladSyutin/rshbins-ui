...
export default function ProofOfIdentity(props) {
  ...
  var continueLoadingDuration = props.continueLoadingDuration !== undefined ? props.continueLoadingDuration : 3000;
  var defaultLastNameValue = props.defaultLastNameValue !== undefined ? props.defaultLastNameValue : '';
  var description = props.description !== undefined
    ? props.description
    : 'Вы давно не заходили в личный кабинет. Пожалуйста, подтвердите личность';
  var heading = props.heading !== undefined ? props.heading : 'Подтверждение личности';
  var lastNameLabel = props.lastNameLabel !== undefined ? props.lastNameLabel : 'Фамилия';
  var lastNameValue = props.lastNameValue;
  var onClose = props.onClose;
  var onContinue = props.onContinue;
  var onLastNameValueChange = props.onLastNameValueChange;
  ...
  var isControlled = lastNameValue !== undefined;
  var uncontrolledValueState = useState(defaultLastNameValue);
  var uncontrolledLastNameValue = uncontrolledValueState[0];
  var setUncontrolledLastNameValue = uncontrolledValueState[1];
  var isContinuePendingState = useState(false);
  var isContinuePending = isContinuePendingState[0];
  var setIsContinuePending = isContinuePendingState[1];
  var continueTimerRef = useRef(null);
  var resolvedLastNameValue = isControlled ? String(lastNameValue != null ? lastNameValue : '') : uncontrolledLastNameValue;
  var resolvedContinueLoading = continueLoading || isContinuePending;
  var isContinueDisabled = resolvedContinueLoading || !isSurnameValueValid(resolvedLastNameValue);
  ...
  function handleLastNameValueChange(nextValue) {
    if (!isControlled) { setUncontrolledLastNameValue(nextValue); }
    if (onLastNameValueChange) { onLastNameValueChange(nextValue); }
  }

  function handleContinue(requestClose) {
    if (resolvedContinueLoading || !isSurnameValueValid(resolvedLastNameValue)) { return; }
    setIsContinuePending(true);
    clearContinueTimer();
    continueTimerRef.current = window.setTimeout(function () {
      continueTimerRef.current = null;
      setIsContinuePending(false);
      if (onContinue) { onContinue(); }
      if (closeOnContinue) { requestClose(); }
    }, continueLoadingDuration);
  }

  useEffect(function () { return clearContinueTimer; }, []);
        ...
        return (
          <div className="rshb-legacy-dialog-modal__actions">
            <Button
              ...
              onClick={function () { handleContinue(ctx.requestClose); }}
              variant="brand"
            >
              {continueLabel}
            </Button>
