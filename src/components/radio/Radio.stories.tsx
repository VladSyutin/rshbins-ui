...
import { Radio, type RadioPreviewState } from './Radio';

const meta = {
  title: 'Modern/Components/Radio',
  component: Radio,
  tags: ['autodocs'],
  args: {
    ...
    return (
      <div style={surfaceStyle}>
        <Radio
          {...args}
          checked={checked}
          name="storybook-radio-playground"
          onChange={(event) => {
  ...
  render: () => (
    <div style={surfaceStyle}>
      <section style={stackStyle}>
        ...
        <p style={metaStyle}>Состояния вспомогательного `_Radio` из Figma: inactive, hover, focused и active.</p>
        <div style={rowStyle}>
          {states.map((state) => (
            <Radio
              ...
              key={state.label}
              label="Контент"
              name={`storybook-radio-overview-${state.label}`}
